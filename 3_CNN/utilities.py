...
from tf_keras.regularizers import L2, L1L2
...
def train_CNN(config, training_config):
    '''
    ...
    class TuneReporterCallback(keras.callbacks.Callback):
        ...
        def on_epoch_end(self, batch, logs={}):
            self.iteration += 1
            train.report(dict(keras_info=logs, mean_accuracy=logs.get("val_accuracy"), mean_loss=logs.get("val_loss")))
    ...
    loss = CategoricalFocalCrossentropy(gamma = config["gamma"])
    model = build_CNN(
                config["input_shape"], 
                loss,
                ...
                print_summary=False,
                kernel_regularizer=L2(config["lambda"])
)
